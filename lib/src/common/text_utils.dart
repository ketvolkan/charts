...
List<TextElement> wrapLabelLines(TextElement labelElement, GraphicsFactory graphicsFactory, num maxWidth, num maxHeight,
  ...
  final labelPartElements = Queue<TextElement>()..addAll(labelElement.text.split(labelDelimiter).map(createTextElement));
  ...
  while (labelPartElements.isNotEmpty && currentLineNumber < maxLines) {
    ...
    var width = currentElement.measurement.horizontalSliceWidth + (currentLineElements.isEmpty ? 0 : currentWidth + delimiterElementWidth);
...
bool _doesLabelFit(bool allowLabelOverflow, TextElement textElement, num maxWidth, TextElement Function(String) createTextElement) {
  ...
  final ellipsizedElementWidth = (createTextElement(ellipsizedText)..textStyle = textElement.textStyle).measurement.horizontalSliceWidth;
