...
const treeMapElementsKey = AttributeKey<Iterable<TreeMapRendererElement<Object>>>('TreeMapRenderer.elements');
...
abstract class BaseTreeMapRenderer<D> extends BaseSeriesRenderer<D> {
  ...
  /// from a treemap node.
  final _treeNodeToRendererElement = <TreeNode<Object>, TreeMapRendererElement<D>>{};
  ...
  void preprocessSeries(List<MutableSeries<D>> seriesList) {
    ...
    for (final series in seriesList) {
      ...
      for (var i = 0; i < count; i++) {
        ...
        _treeNodeToRendererElement[node] = _createRendererElement(series, i, isLeaf: !node.hasChildren);
      }
  ...
  DatumDetails<D> addPositionToDetailsForSeriesDatum(DatumDetails<D> details, SeriesDatum<D> seriesDatum) {
    final bounds = _getRendererElement(seriesDatum.datum as TreeNode<Object>).boundingRect;
    final chartPosition = Point<double>((isRtl ? bounds.left : bounds.right).toDouble(), (bounds.top + (bounds.height / 2)).toDouble());
    return DatumDetails.from(details, chartPosition: NullablePoint.from(chartPosition));
  }
  ...
  void assignMissingColors(Iterable<MutableSeries<D>> seriesList, {required bool emptyCategoryUsesSinglePalette}) {
    for (final series in seriesList) {
      final colorPalettes = StyleFactory.style.getOrderedPalettes(series.data.length);
  ...
  @protected
  num measureForTreeNode(TreeNode<Object> node) => _getRendererElement(node).measure;
  ...
  /// ```
  @protected
  void position(Iterable<TreeNode<Object>> nodes, MutableRectangle boundingRect, num side, num layoutArea) {
    ...
    if (side == boundingRect.height) {
      ...
      for (final node in nodes) {
        final element = _getRendererElement(node);
        final height = min(boundingRect.top + boundingRect.height - top, length > 0 ? (element.area / length) : 0);
      ...
      for (final node in nodes) {
        final element = _getRendererElement(node);
        final width = min(boundingRect.left + boundingRect.width - left, length > 0 ? (element.area / length) : 0);
  ...
  _AnimatedTreeMapRect<D> _createAnimatedTreeMapRect(TreeMapRendererElement<D> element) {
    ...
    final rect = _animatedTreeMapRects.containsKey(key) ? _animatedTreeMapRects.remove(key)! : _AnimatedTreeMapRect<D>(key: key);
  ...
  TreeMapRendererElement<D> _getRendererElement(TreeNode<Object> node) {
    ...
    assert(element != null, 'There is no associated renderer element for $node.');
    return element!;
  }

  void _ensureSingleTree(List<ImmutableSeries<D>> seriesList) {
    assert(seriesList.length <= 1, 'TreeMapRenderer only supports a single series at most.');
  }
}
...
class _AnimatedTreeMapRect<D> {
  ...
  void animateOut() {
    ...
    newTarget.boundingRect = Rectangle(rect.left + (rect.width / 2), rect.top + (rect.height / 2), 0, 0);
  ...
  TreeMapRendererElement<D> getCurrentRect(double animationPercent) {
    ...
    _currentRect!.updateAnimationPercent(_previousRect!, _targetRect, animationPercent);
    return _currentRect!;
  }
}
