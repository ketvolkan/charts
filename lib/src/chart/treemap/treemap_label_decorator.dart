...
class TreeMapLabelDecorator<D> extends TreeMapRendererDecorator<D> {
  ...
  static final _defaultLabelStyle = TextStyleSpec(fontSize: _defaultFontSize, color: Color.black);
  ...
  TreeMapLabelDecorator(
      {TextStyleSpec? labelStyleSpec, this.labelPadding = _defaultLabelPadding, this.allowLabelOverflow = true, this.enableMultiline = false})
      : labelStyleSpec = labelStyleSpec ?? _defaultLabelStyle;

  @override
  void decorate(TreeMapRendererElement<D> rendererElement, ChartCanvas canvas, GraphicsFactory graphicsFactory,
    ...
    final datumLabelStyle = _datumStyle(rendererElement.series.insideLabelStyleAccessorFn, datumIndex, graphicsFactory, defaultStyle: labelStyle);
    ...
    final maxLabelHeight = (renderVertically ? rect.width : rect.height) - (labelPadding * 2);
    final maxLabelWidth = (renderVertically ? rect.height : rect.width) - (labelPadding * 2);
    final multiline = enableMultiline && renderMultiline;
    final parts =
        wrapLabelLines(labelElement, graphicsFactory, maxLabelWidth, maxLabelHeight, allowLabelOverflow: allowLabelOverflow, multiline: multiline);

    for (var index = 0; index < parts.length; index++) {
      final segment = _createLabelSegment(rect, labelHeight, parts[index], index, rtl: rtl, rotate: renderVertically);
  ...
  TextStyle _asTextStyle(GraphicsFactory graphicsFactory, TextStyleSpec labelSpec) => graphicsFactory.createTextPaint()
  ...
  TextStyle _datumStyle(AccessorFn<TextStyleSpec>? labelStyleFn, int datumIndex, GraphicsFactory graphicsFactory, {required TextStyle defaultStyle}) {
    final styleSpec = labelStyleFn?.call(datumIndex);
    return (styleSpec != null) ? _asTextStyle(graphicsFactory, styleSpec) : defaultStyle;
  }

  _TreeMapLabelSegment _createLabelSegment(Rectangle elementBoundingRect, num labelHeight, TextElement labelElement, int position,
    ...
    if (rotate) {
      xOffset = elementBoundingRect.right - labelPadding - 2 * labelElement.textStyle!.fontSize! - labelHeight * position;
    } else if (rtl) {
    ...
    if (!rotate) {
      yOffset = elementBoundingRect.top + labelPadding + (labelHeight * position);
    } else if (rtl) {
    ...
    return _TreeMapLabelSegment(labelElement, xOffset.toInt(), yOffset.toInt(), rotate ? _90DegreeClockwise : 0.0);
  }
}
...
class _TreeMapLabelSegment {
  ...
  _TreeMapLabelSegment(this.text, this.xOffet, this.yOffset, this.rotationAngle);
}
