...
class SliceDiceTreeMapRenderer<D> extends BaseTreeMapRenderer<D> {
  SliceDiceTreeMapRenderer({String? rendererId, TreeMapRendererConfig<D>? config})
      : super(
            config: config ?? TreeMapRendererConfig(tileType: TreeMapTileType.sliceDice),
  ...
  void tile(TreeNode<Object> node) {
    ...
    if (children.isNotEmpty) {
      ...
      position(children, rect, node.depth & 1 == 1 ? rect.height : rect.width, areaForRectangle(rect));
    }
  }
}
