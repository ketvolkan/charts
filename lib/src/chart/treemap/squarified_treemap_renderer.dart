...
class SquarifiedTreeMapRenderer<D> extends BaseTreeMapRenderer<D> {
  ...
  SquarifiedTreeMapRenderer({String? rendererId, TreeMapRendererConfig<D>? config})
      : super(
            config: config ?? TreeMapRendererConfig(tileType: TreeMapTileType.squarified),
...
class _SquarifyRatioAnalyzer {
  ...
  num worst(num width) {
    ...
    return sqArea > 0 ? math.max(sqWidth * rMax * _ratio / sqArea, sqArea / (sqWidth * rMin * _ratio)) : double.infinity;
  }
