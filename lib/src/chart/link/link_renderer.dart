...
import 'package:charts_flutter/common.dart' show Color, LinkOrientation, Link;

import 'link_renderer_config.dart';

const linkElementsKey = AttributeKey<List<LinkRendererElement>>('LinkRenderer.elements');
...
class LinkRenderer<D> extends BaseSeriesRenderer<D> {
  ...
  factory LinkRenderer({String? rendererId, LinkRendererConfig<D>? config}) {
    return LinkRenderer._internal(rendererId: rendererId ?? defaultRendererID, config: config ?? LinkRendererConfig());
  }
  ...
  void preprocessSeries(List<MutableSeries<D>> seriesList) {
    seriesList.forEach((MutableSeries<D> series) {
      ...
      for (var linkIndex = 0; linkIndex < series.data.length; linkIndex++) {
        var element = LinkRendererElement(series.data[linkIndex].link, series.data[linkIndex].orientation, series.data[linkIndex].fillColor);
  ...
  void update(List<ImmutableSeries<D>> seriesList, bool isAnimating) {
    seriesList.forEach((ImmutableSeries<D> series) {
      var elementsList = series.getAttr(linkElementsKey) as List<LinkRendererElement>;
  ...
  void _drawAllLinks(List<LinkRendererElement> links, ChartCanvas canvas) {
    links.forEach((element) => canvas.drawLink(element.link, element.orientation, element.fillColor));
  }
