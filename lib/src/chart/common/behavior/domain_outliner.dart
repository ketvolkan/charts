...
class DomainOutliner<D> implements ChartBehavior<D> {
    ...
    for (var series in seriesList) {
      ...
      if (strokeWidthPxFn != null) {
        series.strokeWidthPxFn = (int? index) {
          ...
          return strokeWidthPx == null ? defaultStrokePx : strokeWidthPx + strokePaddingPx;
        };
      }
    }
  }
  ...
  void attachTo(BaseChart<D> chart) {
    ...
    chart.getSelectionModel(selectionType).addSelectionChangedListener(_selectionChange);
  }

  @override
  void removeFrom(BaseChart<D> chart) {
    chart.getSelectionModel(selectionType).removeSelectionChangedListener(_selectionChange);
